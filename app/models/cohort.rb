...
=end
	...
	def start_reason(start_date = @start_date, end_date = @end_date)
	    ...
	    reason_concept_id = ConceptName.where(["name =?", "REASON FOR ART ELIGIBILITY"]).first.concept_id
...
=end
	...
	def tb_within_the_last_2_yrs(start_date = @start_date, end_date = @end_date)
		tb_concept_id = ConceptName.where(["name =?", "PULMONARY TUBERCULOSIS WITHIN THE LAST 2 YEARS"]).first.concept_id
	...
	def patients_with_start_cause(start_date = @start_date, end_date = @end_date, tb_concept_id = nil)
		...
		cause_concept_id = ConceptName.where(["name =?", "WHO STG CRIT"]).first.concept_id
=begin
PatientProgram.find_by_sql("SELECT patient_id,name,date_enrolled FROM obs
	                        INNER JOIN patient_program p ON p.patient_id = obs.person_id
	                        INNER JOIN patient_state s ON p.patient_program_id = s.patient_program_id
...
=end
	...
	def kaposis_sarcoma(start_date = @start_date, end_date = @end_date)
		tb_concept_id = ConceptName.where(["name =?", "KAPOSIS SARCOMA"]).first.concept_id
...
=end
	...
	def tb_status
		...
		tb_status_concept_id = ConceptName.where(["name =?", 'TB STATUS']).first.concept_id
		hiv_clinic_consultation_encounter_id = EncounterType.where(["name =?", 'HIV CLINIC CONSULTATION']).first.id
=begin
...
=end      
		...
		tb_not_suspected_id = ConceptName.where(["name =?", 'TB NOT SUSPECTED']).first.concept_id
		tb_suspected_id = ConceptName.where(["name =?", 'TB SUSPECTED']).first.concept_id
		tb_confirmed_on_treatment_id = ConceptName.where(["name =?", 'CONFIRMED TB ON TREATMENT']).first.concept_id
		tb_confirmed_not_on_treatment_id = ConceptName.where(["name =?", 'CONFIRMED TB NOT ON TREATMENT']).first.concept_id
...
=end
	...
	def patients_reinitiated_on_art_ever
		Observation.joins([:encounter]).where(["concept_id = ? AND value_coded IN (?) AND encounter.voided = 0 \
			AND DATE_FORMAT(obs_datetime, '%Y-%m-%d') <= ?", ConceptName.where(["name =?", "EVER RECEIVED ART"]).first.concept_id,
	...
	def patients_reinitiated_on_arts
		Observation.joins([:encounter]).where(["concept_id = ? AND value_coded IN (?) AND encounter.voided = 0 \
			...
			ConceptName.where(["name =?", "EVER RECEIVED ART"]).first.concept_id,
			ConceptName.where(["name = 'YES'"]).collect{|c| c.concept_id},
  ...
  def regimens(start_date = @start_date, end_date = @end_date)
    ...
    regimem_given_concept = ConceptName.where(["name =?", "ARV REGIMENS RECEIVED ABSTRACTED CONSTRUCT"]).first
=begin
...
=end
    ...
    ( regimens || [] ).each do | regimen_id, regimen , patient_age |
      ...
      regimen_name = ConceptName.where(["concept_id =?", regimen_id]).first.concept.shortname rescue nil
      if regimen_name.blank?
        regimen_name = ConceptName.where(["concept_id =?", regimen_id]).first.concept.fullname
      end
  ...
  def patients_reinitiated_on_art(start_date = @start_date, end_date = @end_date)
    patients = []
		...
		no_concept = Concep.find_by_name('NO').concept_id
    date_art_last_taken_concept = Concept.find_by_name('DATE ART LAST TAKEN').concept_id

    taken_arvs_concept = Concept.find_by_name('HAS THE PATIENT TAKEN ART IN THE LAST TWO MONTHS').concept_id
    
    defaulted = ConceptName.where(["name =?", "DEFAULTED"])
    defaulted_state = ProgramWorkflowState.where(["concept_id IN (?)", defaulted.map{|c|c.concept_id}]).first.program_workflow_state_id

		treatment_stopped = ConceptName.where(["name =?", "TREATMENT STOPPED"])
    treatment_stopped_state = ProgramWorkflowState.where(["concept_id IN (?)", treatment_stopped.map{|c|c.concept_id}]).first.program_workflow_state_id
   	...
   	PatientProgram.find_by_sql("SELECT patient_id , value_datetime date_art_last_taken,obs_datetime visit_date,value_coded,obs.concept_id concept_id  
                                ...
                                ORDER BY obs.obs_datetime DESC").map do |ob| 
                                	if ob.concept_id.to_s == date_art_last_taken_concept.to_s
																		patient_program_id = PatientProgram.where(["patient_id =?", ob.patient_id]).last.patient_program_id
	...
	def adherence(start_date = @start_date, end_date = @end_date)
		...
		art_adherence = EncounterType.where(["name =?", "ART ADHERENCE"]).last.id
		pills_left_ids = [Concept.find_by_name("AMOUNT OF DRUG BROUGHT TO CLINIC").concept_id,
											  Concept.find_by_name("AMOUNT OF DRUG REMAINING AT HOME").concept_id]
		
		encounters = Encounter.where(["encounter_type = #{art_adherence}"], :limit => 500)#
		...
		encounters.map do |adherence|
		    ...
		    observation = Observation.new
				...
				observation.concept_id = Concept.find_by_name("MISSED HIV DRUG CONSTRUCT").concept_id
  ...
  def kaposis_sarcoma(start_date = @start_date, end_date = @end_date)
    tb_concept_id = Concept.find_by_name("KAPOSIS SARCOMA").concept_id
    self.patients_with_start_cause(start_date, end_date, tb_concept_id)
  end

  def current_episode_of_tb(start_date = @start_date, end_date = @end_date)
    tb_concept_id = Concept.find_by_name("EXTRAPULMONARY TUBERCULOSIS (EPTB)").concept_id
    self.patients_with_start_cause(start_date, end_date, tb_concept_id)
  end
  ...
  def tb_status_with_patient_ids
    ...
    hiv_clinic_consultation_encounter_id = EncounterType.where(["name =?", "HIV CLINIC CONSULTATION"]).last.id
=begin
    status = PatientState.find_by_sql("SELECT * FROM (
                          SELECT e.patient_id,n.name tbstatus,obs_datetime,e.encounter_datetime,s.state
                          FROM patient_state s
...
=end
  ...
  def side_effect_patients(start_date = @start_date, end_date = @end_date)
    side_effect_concept_ids =[Concept.find_by_name('PERIPHERAL NEUROPATHY').concept_id,
                              Concept.find_by_name('HEPATITIS').concept_id,
                              Concept.find_by_name('SKIN RASH').concept_id,
                              Concept.find_by_name('JAUNDICE').concept_id]
    ...
    Observation.where(["encounter_id IN (#{encounter_ids.join(',')})
                     AND concept_id = ? 
                     AND value_coded IN (#{side_effect_concept_ids.join(',')})",concept_id]).group("person_id").length
  end
