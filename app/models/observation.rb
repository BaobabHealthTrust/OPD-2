class Observation < ActiveRecord::Base
  ...
  def self.find_most_common_location(concept_question, answer_string, limit = 10)
    sself.select("COUNT(*) as count, location.name as value").joins(
  ...
  def to_s(tags=[])
      ...
      def self.last_accession_number
        ...
        Observation.joins(["INNER JOIN encounter e ON obs.encounter_id = e.encounter_id"]).where(["accession_number IS NOT NULL AND e.encounter_type = ?",
            lab_orders_encounter_type_id]).order("accession_number + 0").last.accession_number.to_s rescue "00"
      end
      ...
      def self.preferred_diagnoses
        ...
        preferred_diagnosis_concept_ids = GlobalProperty.where(["property =?", property_name]).last.property_value.split(', ') rescue []
