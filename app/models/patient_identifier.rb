class PatientIdentifier < ActiveRecord::Base
  ...
  def self.identifier(patient_id, patient_identifier_type_id)
    patient_identifier = self.where(["patient_id = ? and identifier_type = ?",
        patient_id, patient_identifier_type_id]).select("identifier").first
    return patient_identifier
  end

  def self.next_filing_number(type = 'Filing Number')
    available_numbers = self.where(['identifier_type = ?',
