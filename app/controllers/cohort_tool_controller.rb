class CohortToolController < ApplicationController
  ...
  def changed_to(enc)
    ...
    encounter = Encounter.joins("INNER JOIN obs ON encounter.encounter_id=obs.encounter_id").where(["encounter_type=? AND
      encounter.patient_id=? AND Date(encounter.encounter_datetime)=?", encounter_type,enc.patient_id,
        enc.encounter_datetime.to_date]).group("encounter.encounter_type").order("encounter.encounter_datetime DESC").first
  ...
  def get_visits_by_day(start_date,end_date)
    ...
    Encounter.joins(["INNER JOIN obs     ON obs.encounter_id    = encounter.encounter_id",
        "INNER JOIN patient ON patient.patient_id  = encounter.patient_id"]).where(["obs.voided = 0 AND
        encounter_type IN (?) AND encounter_datetime >=? AND encounter_datetime <=?",
        required_encounters_ids,start_date,end_date]).group("encounter.patient_id,DATE(encounter_datetime)").order("encounter.encounter_datetime ASC")
  end
  ...
  def adherence_over_hundred(quarter="Q1 2009",min_range = nil,max_range=nil,missing_adherence=false)
    ...
    adherence_rates.each{|rate|
      ...
      pill_count = Observation.where("order_id = #{rate.order_id} AND encounter_id = #{rate.encounter_id}
      AND concept_id = #{brought_drug_concept_id} ").first.value_numeric rescue ""
  ...
  def diagnosis_specific_report_menu
    ...
    trauma_concepts.each { |concept|
      ...
      encounters = Encounter.joins([:type, :observations, [:patient => :person]]).where(["encounter_type = ? AND
        encounter.voided = 0 AND value_coded IN (?) AND encounter_datetime >= ? AND encounter_datetime <= ?",
