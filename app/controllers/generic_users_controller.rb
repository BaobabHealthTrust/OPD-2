...
#  verify :method => :post, :only => [ :destroy, :create, :update ],
  ...
  def delete_role
    ...
    unless request.post?
      ...
      user_role =  UserRole.where(["role = ? AND user_id = ?", role,@user.user_id])
      user_role.destroy
  ...
  def view_users
    ...
    @users = User.where(['user_id !=? AND retired = 0',logged_user.id]).order("date_created DESC")
  ...
  def print_login_password
    print_and_redirect("/users/login_label?user_id=#{params[:user_id]}", "/logout")
  end
