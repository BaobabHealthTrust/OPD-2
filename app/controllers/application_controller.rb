class ApplicationController < GenericApplicationController
  ...
  # Try to find the next task for the patient at the given location
	...
	def is_encounter_available(patient, encounter_type, session_date)
        ...
        patient.id, EncounterType.find_by_name(encounter_type).id, session_date]).order("encounter_datetime DESC").limit(1).first
	...
	def encounter_available_ever(patient, encounter_type)
		is_available = false
		encounter_available = Encounter.where(["patient_id = ? AND encounter_type = ?",
        patient.id, EncounterType.find_by_name(encounter_type).id]).order("encounter_datetime DESC").limit(1).first
  ...
  def hiv_program
  	program = PatientProgram.where({:patient_id => @patient.id,
        :program_id => Program.find_by_name('HIV PROGRAM').id}).first rescue nil
    unless program.nil?
